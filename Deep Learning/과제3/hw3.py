...
class nn_activation_layer:
    ...
    def forward(self,x):
        ...
        for i in range(a):
            for j in range(b):
                e[i][j] = math.exp(x[i][j] * -1)
        sigmoid = np.zeros((a, b))
        for i in range(a):
            for j in range(b):
                sigmoid[i][j] = 1 / (1 + e[i][j])
    ...
    def backprop(self,x,dLdy):
        ...
        for i in range(a):
            for j in range(b):
                dLdx[i][j] = dLdy[i][j] * sigmoid[i][j] * (1 - sigmoid[i][j])
...
class nn_softmax_layer:
    ...
    def forward(self,x):
        ...
        for i in range (a):
            for j in range(b):
                soft[i][j] = exp_x[i][j] / sum_exp_x[i]
    ...
    def backprop(self,x,dLdy):
        ...
        sof = np.zeros((sum_exp_x.shape))
        for i in range(a):
            sof[i] = exp_x[i][0] * exp_x[i][1] / math.pow(sum_exp_x[i],2)

        for i in range(a):
            for j in range(b):
                dLdx[i][j] = dLdy[i][j] * sof[i]
...
class nn_cross_entropy_layer:
    ...
    def forward(self,x,y):
        ...
        for i in range(a):
            Loss[i] = (1-y[i]) * math.log(x[i][0]) + y[i] * math.log(x[i][1])

        sum_loss = np.sum(Loss) * (-1/a)
    ...
    def backprop(self,x,y):
        ...
        for i in range(a):
            for j in range(b):
                dLdx[i][j] = ((1 - y[i]) / (x[i][j] - j) + y[i] / (x[i][j] - 1 + j)) * (-1/a)
