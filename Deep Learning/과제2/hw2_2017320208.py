...
def cross_entropy_softmax_loss(Wb, x, y, num_class, n, feat_dim):
    ...
    score = W@x + b
    exp_s = np.zeros((num_class,n))

    for i in range(num_class):                  ###자연상수의 지수승으로 나타내줌
        for j in range(n):
            exp_s[i,j] = math.exp(score[i,j])

    tot_exp = np.sum(exp_s, axis= 0)

    answer = np.zeros((n,))

    for i in range(n):
         answer[i] = math.log(exp_s[y[i],i] / tot_exp[i])

    cross_entropy_loss = np.sum(answer, axis=0) * (-1)
...
def svm_loss(Wb, x, y, num_class, n, feat_dim):
    ...
    classify = np.max(score, axis = 0)           ###각 data에 대해서 가장 점수가 높게 나온 class의 점수의 vector(1xn vector)
    Loss = 0
    answer = np.zeros((n,))

    for i in range(n) :
        answer[i] = score[y[i],i]
    for i in range(n) :
        if(answer[i] >= classify[i]+1):
            Loss += 0
        else:
            Loss += classify[i]+1 - answer[i]
