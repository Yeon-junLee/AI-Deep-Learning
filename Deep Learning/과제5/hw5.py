...
class nn_mnist_classifier:
    ...
    def update_weights(self):
        ...
        self.v_w_fc2 = lr * (friction * self.v_w_fc2 + (1 - friction) * dldw_fc2) * (-1)
        self.v_b_fc2 = lr * (friction * self.v_b_fc2 + (1 - friction) * dldb_fc2) * (-1)

        self.v_w_fc1 = lr * (friction * self.v_w_fc1 + (1 - friction) * dldw_fc1) * (-1)
        self.v_b_fc1 = lr * (friction * self.v_b_fc1 + (1 - friction) * dldb_fc1) * (-1)

        self.v_w_cv1 = lr * (friction * self.v_w_cv1 + (1 - friction) * dldw_cv1) * (-1)
        self.v_b_cv1 = lr * (friction * self.v_b_cv1 + (1 - friction) * dldb_cv1) * (-1)

        # using v, perform weight update for each layer
        self.fc2.update_weights(self.v_w_fc2,self.v_b_fc2)
        self.fc1.update_weights(self.v_w_fc1,self.v_b_fc2)
        self.conv_layer_1.update_weights(self.v_w_cv1,self.v_b_cv1)
        ########################
        # Q3 ends here
        ########################
